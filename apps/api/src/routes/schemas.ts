...
export const cryptoToBankQuoteSchema = z.object({
  ...
  (data) => data.fromAmount !== undefined || data.toAmount !== undefined,
...
export const bankResolutionSchema = z.object({
  countryCode: z.string().length(2).default("NG"),
  bankCode: z.string().optional(),
  bankName: z.string().optional(),
  accountNumber: z.string().min(10).max(10)
}).refine(
  (data) => (data.bankCode?.trim() ?? "") || (data.bankName?.trim() ?? ""),
